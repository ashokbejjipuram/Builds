...
namespace IMPALLibrary
{
    public class EinvAuthGen
    {
        ...
        public void EinvoiceAuthentication(string JSONData, string BranchCode, string DocNumber, string Indicator, string Action, string SellerGSTIN, string DocType, string DocDate, DataSet ds)
        {
            ...
            using (TransactionScope scope = DataAccess.NewWriteTransactionScope())
            {
                ...
                if (!(BDOstatus.Item2 == "" || BDOstatus.Item3 == "" || BDOstatus.Item4 == "") || !(einvresp.bdoAuthtoken == "" || decrSEK == "" || decrSEKplain == ""))
                {
                    ...
                    if (!(EncrytedJSON == "" || EncrytedJSON == null))
                    {
                        ...
                        if (response.Contains("Duplicate IRN") || response.Contains("DUPIRN") || response.Contains("Duplicate"))
                        {
                            ...
                            if (jobj["InfoDtls"] != null)
                            {
                                ...
                                EndPointURL = EndPointURL + "doctype=" + DocType + "&docnum=" + DocNumber + "&docdate=" + DocDate;
                                ...
                                httpWebRequest.Headers.Add("gstin", SellerGSTIN);
                                ...
                                {
                                    einvresp = new EinvResponse(status, string.Empty, string.Empty, string.Empty, jobj["ErrorMsg"].ToString().Replace("\"", "").Trim());
                                    obj = new Exception(String.Format("{0} : {1} : Error Msg : {2}", "Einvoicing JSON Data Upload Failed", DocNumber, jobj["ErrorMsg"].ToString().Replace("\"", "").Trim()));
                                    throw obj;
                                }
                                else
                                {
                                    ...
                                    {
                                        byte[] imageData = genQRcode.GenerateInvQRCode(jobj["SignedQRCode"].ToString().Replace("\"", "").Trim());
                                        ...
                                        salesTransactions.AddEinvoiceIRNDetails(BranchCode, DocNumber, jobj["AckNo"].ToString().Replace("\"", "").Trim(), jobj["AckDt"].ToString().Replace("\"", "").Trim(),
                                                                             ...
                                                                             jobj["EwbDt"].ToString().Replace("\"", "").Trim(), "Duplicate IRN",
                                                                             jobj["SignedInvoice"].ToString().Replace("\"", "").Trim(), jobj["SignedQRCode"].ToString().Replace("\"", "").Trim(), imageData, "1");
                                    }
                                    else
                                    {
                            ...
                            else
                            {
                                ...
                                if (!response.Contains("Duplicate invoice number"))
                                {
                                    if (!(BDOstatus.Item2 == "" || BDOstatus.Item3 == "" || BDOstatus.Item4 == ""))
                                        Finaloutput = objEncrDecr.DecryptBySymmetricKey(jobj["Data"].ToString().Replace("\"", "").Trim(), BDOstatus.Item4);
                                    else if (!(bdoAuthtoken == "" || decrSEK == "" || decrSEKplain == ""))
                        ...
                        else
                        {
                            ...
                            if (status.Trim() == "0")
                            {
                                ...
                                if (Action == "GENIRN")
                                {
                                    if (jobj["ErrorMsg"] != null)
                                    {
                                        if (jobj["ErrorMsg"].ToString().ToLower().Contains("inactive or cancelled"))
                                        {
                                            GenerateJSON objGenJsonData = new GenerateJSON();

                                            EinvoiceAuthenticationB2C(objGenJsonData.GenerateInvoiceJSONDataB2C(ds, BranchCode), BranchCode, DocNumber, "1", "NOIRN");
                                        }
                                        else if (jobj["ErrorMsg"].ToString().ToLower().Contains("invalid shipto gstin") || (jobj["ErrorMsg"].ToString().ToLower().Contains("invalid gstin") && !jobj["ErrorMsg"].ToString().ToLower().Contains("onboard")))
                                        {
                                            GenerateJSON objGenJsonData = new GenerateJSON();

                                            EinvoiceAuthenticationB2C(objGenJsonData.GenerateInvoiceJSONDataB2C(ds, BranchCode), BranchCode, DocNumber, "0", "NOIRN");
                                        }
                                        else
                                        {
                                            einvresp = new EinvResponse(status, string.Empty, string.Empty, string.Empty, jobj["ErrorMsg"].ToString().Replace("\"", "").Trim());
                                    ...
                                    {
                                        if (jobj["Error"].ToString().ToLower().Contains("inactive or cancelled"))
                                        {
                                            GenerateJSON objGenJsonData = new GenerateJSON();

                                            EinvoiceAuthenticationB2C(objGenJsonData.GenerateInvoiceJSONDataB2C(ds, BranchCode), BranchCode, DocNumber, "1", "NOIRN");
                                        }
                                        else if (jobj["Error"].ToString().ToLower().Contains("invalid shipto gstin") || (jobj["Error"].ToString().ToLower().Contains("invalid gstin") && !jobj["Error"].ToString().ToLower().Contains("onboard")))
                                        {
                                            GenerateJSON objGenJsonData = new GenerateJSON();
                            ...
                            else
                            {
                                ...
                                else if (Action == "GENIRN")
                                {
                                    ...
                                    else
                                    {
                                        ...
                                        else if (jobj["ErrorMsg"].ToString().ToLower().Contains("invalid shipto gstin") || jobj["ErrorMsg"].ToString().ToLower().Contains("invalid gstin"))
                                        {
                                            GenerateJSON objGenJsonData = new GenerateJSON();
        ...
        {
            GenerateQRcode genQRcode = new GenerateQRcode();
            byte[] imageData = genQRcode.GenerateInvQRCodeB2C(B2CQRCodeData);

            SalesTransactions salesTransactions = new SalesTransactions();
            salesTransactions.AddEinvoiceDetails(BranchCode, DocNumber, "UNREGISTERED", "", "", "", "", "", "", "", "", B2CQRCodeData, imageData, Indicator);
        }
