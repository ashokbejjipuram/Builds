...
{
    public partial class DownloadToBranch : System.Web.UI.Page
    {
        #region Private Variables

        private const string serverDownloadFolder = "Downloads-ToBranch";
        ...
        protected void DownloadBranchTables()
        {
            ...
            try
            {
                if (downloadResult.Contains(serverDownloadFolder))
                {
                    ...
                    dataSetBranchTables = download.DownloadPaymentToBranchTableDetails((string)Session["BranchCode"]);
                    ...
                    foreach (DataTable table in dataSetBranchTables.Tables)
                    {
                        foreach (DataRow row in table.Rows)
                        {
                            ...
                            if (download.DownloadPaymentToBranchCorporateData(downloadResult, (string)Session["BranchCode"], tableName, tableColumnsCount))
                            {
                                ...
                                download.DownloadPaymentToBranchUpdateCorporateData(downloadResult, (string)Session["BranchCode"], tableName, tableColumnsCount);
                            }
                            else
                            {
                                Session["DownloadTableDetails"] = Session["DownloadTableDetails"].ToString() +
