...
{
    public partial class DownloadAllBranch : System.Web.UI.Page
    {
        ...
        protected void btnDownload_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                ltrProgressBar.Text = "<div id=\"divShadowForBar\" style=\"display:block\"></div><div id=\"divAjaxProgressBarLoader\" style=\"display:block\"></div>";

                dataSetBranches = download.DownloadBranchDetails();

                foreach (DataTable branch in dataSetBranches.Tables)
                {
                    foreach (DataRow branchname in branch.Rows)
                    {
                        BranchName = branchname[0].ToString();

                        Session["BranchCode"] = branchname[0].ToString();                        
        ...
        protected void DownloadBranchTables()
        {
            ...
            try
            {
                ...
                if (downloadResult.Contains(serverDownloadFolder))
                {
                    ...
                    totalThreadCount = 6000;

                    Session["TotalDownloadCount"] = 6000;
                    ...
                    Session["DownloadTableDetails"] = Session["DownloadTableDetails"].ToString() +
                                                                            ...
                                                                            string.Format("Download Branch {0} - success", Session["BranchCode"]), "");
