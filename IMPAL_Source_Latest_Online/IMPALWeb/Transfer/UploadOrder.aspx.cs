...
namespace IMPALWeb.Transfer
{
    public partial class UploadOrder : System.Web.UI.Page
    {
        ...
        protected void btnHOOrderUpload_Click(object sender, EventArgs e)
        {
            Type Source = System.Reflection.MethodBase.GetCurrentMethod().DeclaringType;
            try
            {
                btnHOOrderUpload.Visible = false;
                ...
                if (btnFileUpload.HasFile)
                {
                    ...
                    if (File.Exists(filePath + "\\" + fileName))
                    {
                        ...
                        Session["UploadOrderDetails"] = upload.UploadHOorderDataFromTextFile(filePath, fileName, branchCode);
                        FileStatusMsg.Text = Session["UploadOrderDetails"].ToString();                        
                    }
                    else
                    {
        ...
        {
            Server.ClearError();
            Response.Redirect("UploadOrder.aspx", false);
