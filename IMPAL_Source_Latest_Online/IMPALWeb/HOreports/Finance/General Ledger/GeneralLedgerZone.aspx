<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="GeneralLedgerZone.aspx.cs"
    Inherits="IMPALWeb.Reports.Finance.General_Ledger.GeneralLedgerZone" MasterPageFile="~/Main.Master" %>
...
<asp:Content ID="Content" ContentPlaceHolderID="CPHDetails" runat="server">
    ...
    <script language="javascript" type="text/javascript">
        function FnValidate() {
            ...
            let str = "sridhar"; //,cfo,mrsriram
            if (!(str.includes('<%=Session["UserID"]%>') || str.includes('<%=Session["auditsouth"]%>') || str.includes('<%=Session["auditnorth"]%>')
                || str.includes('<%=Session["auditeast"]%>') || str.includes('<%=Session["auditwest"]%>'))) {
                if (ddlGLMain.value == "") {
