...
{
    public partial class GeneralLedgerZone : System.Web.UI.Page
    {
        #region Declaration
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                if (!IsPostBack)
                {
                    ...
                    if (Session["UserID"].ToString() == "auditsouth")
                        ZoneCode = 1;
                    else if (Session["UserID"].ToString() == "auditnorth")
                        ZoneCode = 2;
                    else if (Session["UserID"].ToString() == "auditeast")
                        ZoneCode = 3;
                    else if (Session["UserID"].ToString() == "auditwest")
                        ZoneCode = 4;

                    ddlBranch.DataSource = oFinance.GetGLBranchesZone(ZoneCode);
        ...
        private void CallCrystalReport()
        {
            ...
            try
            {
                if (!string.IsNullOrEmpty(strBranchCode))
                {
                    ...
                    oFinance.CalculateGeneralLedgerZone(oProp);
                }
            }
            catch (Exception exp)
            {
        ...
        private void LoadAccPeriodDDL()
        {
            ...
            try
            {
                ...
                {
                    AccountingPeriods oAcc = new AccountingPeriods();
                    ddlAccPeriod.DataSource = oAcc.GetAccountingPeriod(0, "GLAUDIT", strBranchCode);
                    ddlAccPeriod.DataTextField = "Desc";
                    ddlAccPeriod.DataValueField = "AccPeriodCode";
                    ddlAccPeriod.DataBind();
                }
        ...
        protected void btnReport_Click(object sender, EventArgs e)
        {
            ...
            //cmd = ImpalDB.GetSqlStringCommand("select top 1 Datediff(ss, datestamp, GETDATE()) from Rpt_ExecCount_Daily WITH (NOLOCK) where BranchCode = '" + Session["BranchCode"].ToString() + "' and reportname = 'GLreportHO' order by datestamp desc");
        ...
        protected void GenerateAndExportReport(string fileType)
        {
            ...
            {
                case "Report":
                    rptCrystal.ReportName = "GLreportHO";
                    break;
                case "Summary":
                    rptCrystal.ReportName = "GLsummaryHO";
                    break;
            }

            if (!ddlBranch.SelectedValue.Equals("CRP"))
            {
                if (ddlReportType.SelectedValue == "Report")
                    rptCrystal.RecordSelectionFormula = "{V_General_Ledger_Detail_HO.Branch_Code}='" + ddlBranch.SelectedValue + "'";
                else
                    rptCrystal.RecordSelectionFormula = "{V_General_Ledger_Detail_Summ_HO.Branch_Code}='" + ddlBranch.SelectedValue + "'";
            }
            else
            {
                if (ddlReportType.SelectedValue == "Report")
                    rptCrystal.RecordSelectionFormula = "{V_General_Ledger_Detail_HO.Report_Branch}='" + ddlBranch.SelectedValue + "'";
                else
                    rptCrystal.RecordSelectionFormula = "{V_General_Ledger_Detail_Summ_HO.Report_Branch}='" + ddlBranch.SelectedValue + "'";
            }
        ...
        protected void btnBack_Click(object sender, EventArgs e)
        {
            ...
            {
                Server.ClearError();
                Response.Redirect("GeneralLedgerZone.aspx", false);
