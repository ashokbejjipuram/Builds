...
var CtrlGridRowIdPrefix = "ctl00_CPHDetails_grvCashAndBankPaymentNew_ctl02_";
...
function CalcTotalValue() {
    var gvDrv = document.getElementById(CtrlIdPrefix + "grvCashAndBankPaymentNew");
    ...
    for (i = 1; i < gvDrv.rows.length - 1; i++) {
        ...
        if (txtAmount.value != "") {
            totalAmount = totalAmount + parseFloat(parseFloat(txtAmount.value).toFixed(2));
        }
    }

    document.getElementById(CtrlIdPrefix + "hdnTotalAmount").value = parseFloat(totalAmount).toFixed(2);
    gvDrv.rows[gvDrv.rows.length - 1].cells[3].innerHTML = parseFloat(totalAmount).toFixed(2);
}
