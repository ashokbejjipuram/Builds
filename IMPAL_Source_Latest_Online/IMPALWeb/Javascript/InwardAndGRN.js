...
function AlphaNumericOnlyGSTIN() {
    var AsciiValue = event.keyCode
    if ((AsciiValue >= 48 && AsciiValue <= 57) || (AsciiValue == 8 || AsciiValue == 127) || (AsciiValue >= 65 && AsciiValue <= 90) || (AsciiValue >= 97 && AsciiValue <= 122))
        event.returnValue = true;
    else
        event.returnValue = false;
}
...
function InwardHeaderValidation() {
    ...
    var SupplierGSTIN = document.getElementById(CtrlIdPrefix + "txtSupplierGSTIN");
    ...
    if (TransType == "171") //Free Of Cost
    {
        ...
        if (SupplierGSTIN.value.trim() == "") {
            alert("Supplier Plant GSTIN should not be empty.");
            SupplierGSTIN.value = "";
            SupplierGSTIN.focus();
            return false;
        }

        if (SupplyPlnt == "0" || SupplyPlnt.trim() == "") {
        ...
        if (parseFloat(invoiceValue.value) > 0) {
            ...
            if (OSIndicator == "L") {
                ...
                if (parseFloat(invoiceValue.value) > 0 || parseFloat(IGSTValue.value) > 0) {
                    if (parseFloat(IGSTValue.value.trim()) >= parseFloat(invoiceValue.value.trim())) {
                        alert("Header IGST Value Should be less than the Invoice value");
                        IGSTValue.value = "0.00";
                        IGSTValue.focus();
