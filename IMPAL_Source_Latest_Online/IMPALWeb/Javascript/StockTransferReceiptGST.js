...
function AutoFillReference(id, i) {
  
    if (i == "1")
    {
        var Id = id.replace("txtCCWHNo", "");
        var txtCCWHNo = document.getElementById(id);   
        var txtRefInvNumber = document.getElementById(Id + "txtInvoiceNumber");  
        
        txtRefInvNumber.value = txtCCWHNo.value; 
    } 
    else
    {
        var Id = id.replace("txtRefStockTransfeDate", "");
        var txtCCWHDate = document.getElementById(id);
        var txtRefInvDate = document.getElementById(Id + "txtInvoiceDate");  
        
        txtRefInvDate.value = txtCCWHDate.value;
    }
}
...
function StockTransferReceiptSubmit() {
    if (StockTransferReceiptHeaderValidation()) {
...
function GetTotal() {
    ...
    if (rowscount >= 3) {
        for (k = 1; k < rowscount - 1; k++) {
            ...
            txtPartNoId = row.cells[4].childNodes[0];

            var browserName = navigator.appName;
            if (browserName == 'Netscape') {
                txtPartNoId = row.cells[4].childNodes[1];
            }

            txtPartNo = txtPartNoId.childNodes[1];            
            txtCostPr = row.cells[7].children[0];
            txtRecQty = row.cells[8].children[0];
            ...
            if (txtGSTPer.value != "") {
                ...
                else if (txtTaxGroupCode.value == "5" && parseFloat(TaxValue) != 18 && interStateStatus.value != "1") {
    ...
    if (idDate != '') {
        ...
        if (!status) {
            ...
            if (RecDate > OriginalRecDate) {
                alert('Original Receipt Date (' + idDate + ') should be less than the Reference Stock Transfer Date.');
                document.getElementById(id).value = "";
                document.getElementById(id).focus();
                return false;
            }
