...
var CtrlGridRowIdPrefix = "ctl00_CPHDetails_grvInterBranchPayment_ctl02_";
...
function ValidAmountLimit(id) {    
    TransactionAmount = document.getElementById(CtrlIdPrefix + "txtTransactionAmount").value;
    
    var count = TransactionAmount.split('.');

    if (count.length - 1 > 1) {
        document.getElementById(CtrlIdPrefix + "txtTransactionAmount").value = "";
        return false;
    }

    var ddlPayment = document.getElementById(CtrlIdPrefix + "ddlPayment").value;

    if (ddlPayment == "0") {
        alert("Please Select the Payment Type");
        document.getElementById(CtrlIdPrefix + "txtTransactionAmount").value = "";
        document.getElementById(CtrlIdPrefix + "ddlPayment").focus();
        return false;
    }

    if (ddlPayment == "P") {
        if (parseFloat(TransactionAmount) > 10000 && id != "mpcor") {
            alert("Amount More than 10000 Cannot be Paid Directly. Please Book a Liability for the same");
            document.getElementById(CtrlIdPrefix + "txtRemarks").value = "";
            document.getElementById(CtrlIdPrefix + "txtTransactionAmount").value = "";
            document.getElementById(CtrlIdPrefix + "txtRemarks").disabled = true;
            document.getElementById(CtrlIdPrefix + "btnReportTransaction").disabled = true;
            document.getElementById(CtrlIdPrefix + "BankAccNo_imgPopup").disabled = true;
            document.getElementById(CtrlIdPrefix + "ddlPayment").disabled = true;
            document.getElementById(CtrlIdPrefix + "ddlCashCheque").disabled = true;
            document.getElementById(CtrlIdPrefix + "txtNoOfTransactions").disabled = true;
            return false;
        }
        else {
            document.getElementById(CtrlIdPrefix + "txtRemarks").disabled = false;
            document.getElementById(CtrlIdPrefix + "btnReportTransaction").disabled = false;
            document.getElementById(CtrlIdPrefix + "BankAccNo_imgPopup").disabled = false;
            document.getElementById(CtrlIdPrefix + "ddlPayment").disabled = false;
            document.getElementById(CtrlIdPrefix + "ddlCashCheque").disabled = false;
            document.getElementById(CtrlIdPrefix + "txtNoOfTransactions").disabled = false;
            return true;
        }
    }
}
...
function ValidateTransactionFields() {
    ...
    var ddlPayment = document.getElementById(CtrlIdPrefix + "ddlPayment").value;
    var ddlBranchOthers = document.getElementById(CtrlIdPrefix + "ddlBranchOthers").value;
    ...
    if (ddlBranchOthers == "0") {
        alert("Please Select the Inter Branch");
        document.getElementById(CtrlIdPrefix + "ddlBranchOthers").focus();
        return false;
    }
    ...
    if (validate(document.getElementById(CtrlIdPrefix + "txtNoOfTransactions").value, "No of Transactions ")) {
        document.getElementById(CtrlIdPrefix + "txtNoOfTransactions").focus();
        return false;
    }
    
    if (document.getElementById(CtrlIdPrefix + "txtNoOfTransactions").value != "" && isNaN(document.getElementById(CtrlIdPrefix + "txtNoOfTransactions").value)) {
        alert("No of Transactions should be number only");
        return false;
    }
    
    if (document.getElementById(CtrlIdPrefix + "txtNoOfTransactions").value == 0) {
        alert("No of Transactions cannot be Zero");
        document.getElementById(CtrlIdPrefix + "txtNoOfTransactions").focus();
        return false;
    }
...
function GridValidate() {
    var gvDrv = document.getElementById(CtrlIdPrefix + "grvInterBranchPayment");
    var CashCheque = document.getElementById(CtrlIdPrefix + "ddlCashCheque").value;
    ...
    if (parseFloat(totalAmount) > parseFloat(TransAmount)) {
        alert("Details Total Amount exceeds the Header Transaction Amount by Rs. " + round((parseFloat(totalAmount) - parseFloat(TransAmount)), 2));
        return false;
    }
    ...
    if (parseFloat(TransAmount) != parseFloat(totalAmount)) {
        ...
        if (parseFloat(totalAmount) < parseFloat(TransAmount)) {
            alert("Details Amount is Not Tallied with the Header Transaction Amount");
            return false;
        }
    }
    else {
    ...
    if (parseFloat(TransAmount) == parseFloat(totalAmount)) {
        var result = confirm("Transaction Amount is Tallied with the Total Amount on Row Number " + TotRow + ". Do You want to Submit?");
        if (result) {

            //            for (k = TotRow; k > gvDrv.rows.length - TotRow - 1; k++) {
            //                gvDrv.deleteRow(k);
            //            }
