...
function funFYRcvReceiptSubmit() {
    ...
    if (funReceiptHeaderValidation()) {
        ...
        if (GridCheck()) {
            var AdvanceAmount = document.getElementById(CtrlIdPrefix + "txtAdvanceAmount");
            var TotalBalAmount = document.getElementById(CtrlIdPrefix + "txtTotalBalanceAmount").value.trim();
            if (parseFloat(TotalBalAmount) == 0) {
                document.getElementById(CtrlIdPrefix + "txtTotalBalanceAmount").disabled = true;
                return true; //post back gets happen for save/update.
            }
            else {
                if (document.getElementById(CtrlIdPrefix + 'ddlModeOfReceipt').value != 'CA') {
                    if (AdvanceAmount.value.trim() == "" || AdvanceAmount.value.trim() == null) {
                        alert("Advance Amount should not be Empty.");
                        AdvanceAmount.focus();
                        return false;
                    }

                    alert("Collection Amount does not match with the Invoice values Adjustment. Please Make Sure if any Pending Invoices are Missing for Adjustment or Advance Amount Transferred from Dealer.");
...
function GetTotal() {
    ...
    var CustomerAdvanceAmount = document.getElementById(CtrlIdPrefix + "txtAdvanceAmount").value.trim();
    ...
    if (isNaN(CustomerAdvanceAmount) || CustomerAdvanceAmount == "")
        CustomerAdvanceAmount = 0.00;

    document.getElementById(CtrlIdPrefix + "txtTotalBalanceAmount").value = ((parseFloat(CustomerCollectionAmount) - parseFloat(CustomerAdvanceAmount)) - parseFloat(totalCollectionAmount)).toFixed(2);
}
