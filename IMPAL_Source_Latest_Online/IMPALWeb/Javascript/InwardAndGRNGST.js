...
//    validationDate.setDate(day);
//    validationDate.setMonth(month);
//    validationDate.setFullYear(year);
//    validationDate.setHours(0);
//    validationDate.setMinutes(0);
//    validationDate.setSeconds(0);
//    validationDate.setMilliseconds(0);
...
function InwardHeaderValidation() {    
    ...
    var ddlFrgtInd = document.getElementById(CtrlIdPrefix + "ddlFrightIndicator");
    ...
    if (TransType == "171") //Free Of Cost
    {
        ...
        if ((IGSTValue.value.trim() != "") && (IGSTValue.value.trim() != "0.00")) {
...
function InwardEntrySubmit() {
    if (InwardHeaderValidation()) {
        ...
        if (GridCheck()) {
            ...
            freightamount = document.getElementById(CtrlIdPrefix + "txtFrightAmount").value;
...
//            }
            ...
            tot_value = parseFloat(tot_value);  //+ parseFloat(freightamount) + parseFloat(insurance) + parseFloat(postalcharges) + parseFloat(couponcharges);
            ...
            var differenceAmount = parseFloat(tot_value) - (((parseFloat(ItemsCostValueSummation) * (1 - (insurance_Value))) * (1 - disc_Value)) + ((parseFloat(ItemsTaxValueSummation) * (1 - (insurance_Value))) * (1 - disc_Value)));
...
function CheckGRNGridDuringUpdate() {
    ...
    for (var row = 0; row <= ctrlArrMain.length - 1; row++) {
        ...
        //        alert(ctrlArr[1]);
        //        alert(ctrlArr[2]);
        //        alert(document.getElementById(ctrlArr[0]).value);
...
function funGRNAcceptedQtyValidation(InwardQtyID, AcceptedQtyID, ShoratageQtyID) {
    ...
    if ((InwardQty != null) && (InwardQty != undefined) && (InwardQty.value != 0) && (InwardQty.value.trim() != "")) {
        //alert(AcceptedQty.value);
        //alert(parseFloat(AcceptedQty.value.trim()));

        //if ((AcceptedQty != null) && (AcceptedQty != undefined) && (AcceptedQty.value != 0))
        if ((AcceptedQty != null) && (AcceptedQty != undefined)) {
            if (parseFloat(AcceptedQty.value.trim()) > parseFloat(InwardQty.value.trim())) {
                ...
                if (parseFloat(ShortageQty.value) > 0) {
                    ...
                    document.getElementById("ctl00_CPHDetails_grvItemDetails_" + strid[3] + '_txtWareHouseNo').disabled = false;
                    document.getElementById("ctl00_CPHDetails_grvItemDetails_" + strid[3] + '_txtWarehouseDate').disabled = false;
                    //document.getElementById("ctl00_CPHDetails_grvItemDetails_" + strid[3] + '_ImgWareHouseDate1').disabled = false;
                }
                else {
                    document.getElementById("ctl00_CPHDetails_grvItemDetails_" + strid[3] + '_txtWareHouseNo').value = "";
                    document.getElementById("ctl00_CPHDetails_grvItemDetails_" + strid[3] + '_txtWarehouseDate').value = "";

                    document.getElementById("ctl00_CPHDetails_grvItemDetails_" + strid[3] + '_txtWareHouseNo').disabled = true;
                    document.getElementById("ctl00_CPHDetails_grvItemDetails_" + strid[3] + '_txtWarehouseDate').disabled = true;
                    //document.getElementById("ctl00_CPHDetails_grvItemDetails_" + strid[3] + '_ImgWareHouseDate1').disabled = true;
                }
...
//------- For Report Salesman Dealer Date Validation
