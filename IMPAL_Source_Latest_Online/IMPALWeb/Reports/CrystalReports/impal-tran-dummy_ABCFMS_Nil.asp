...
<head>
<title>Impal - Dummy [ABCFMS]</title>
<body>
<center>
	<font color="white" face="Arial" size="4" style="BACKGROUND-COLOR: #336699">
	<strong>INVENTORY - Dummy [ ABCFMS ]</strong>
	</font>
</center>
...
<% 	DIM hhidval
	...
	if Request.form("cbosupplierpartnumber") <> "" then
		sqlquery="select Indent_Number from Item_Worksheet_ABCFMS_Nil"
...
'		sqlins = "select count(item_code) from item_worksheet where item_code = '" & Request.Form("txtitcode") & "'"
		sqlins = "select count(supplier_part_number) from Item_Worksheet_ABCFMS_Nil where supplier_part_number = '" & Request.form("cbosupplierpartnumber") & "'"
...
'		else
    ...
    dim sqlquerynil
    set adors=nothing
    sqlquerynil ="select isnull(count(*),0) from Item_Worksheet_ABCFMS_Nil where item_code = '" & Request.form("txtitcode") & "' and Supplier_Part_Number ='" & request.form("cbosupplierpartnumber") & "' and " & _
    ...
    if wscount = 0 then
			sqlqueryins="Insert into Item_Worksheet_ABCFMS_Nil(Item_Code,Item_Short_Description,"
...
<%
		...
		sqlquery = "select Vehicle_Type_Description from V_Itemworksheet_ABCFMS_Nil_Vehicle where to_order_qty >0 and indent_number = '" & ponumber1 & "' group by Vehicle_Type_Description"
	...
	while not adors1.eof
		sqlquery = "select count(*) from V_Itemworksheet_ABCFMS_Nil_Vehicle where to_order_qty >0 and indent_number = '" & ponumber1 & "' and Vehicle_Type_Description='" & adors1(0)& "'"
' response.Write "<br> " & sqlquery 		
		...
		<%
'response.Write "<br> partno " & Request.form("hidpartnumber1")
		i = 0
		for i = 0 to reccount
...
'				sqlquery = sqlquery & " packing_quantity,0 as accepted,'',item_code from Item_Worksheet_ABCFMS_Nil "
'				sqlquery = sqlquery & " where To_Order_Qty > 0 and indent_number = '" & ponumber1 & "' order by supplier_part_number"
				...
				sqlquery = sqlquery & " Curr_Month as currentmthsales, "
				sqlquery = sqlquery & " To_Order_qty, "
				sqlquery = sqlquery & " packing_quantity,0 as accepted,'',item_code ,ABCFMS_Status,Vehicle_Type_Description from V_Itemworksheet_ABCFMS_Nil_Vehicle "
'				sqlquery = sqlquery & " where To_Order_Qty > 0 and indent_number = '" & ponumber1 & "'  order by supplier_part_number"
...
'	Response.Write "i : " & i
	set adors=nothing
	sqlquery = "delete from Item_Worksheet_ABCFMS_Nil where to_order_qty = 0 and indent_number = '" & Request.Form("hidpartnumber1") & "'"
	call sqlexec(sqlquery) 
%>
</CENTER>
</TABLE>
...
-->
    ...
    <input type="hidden" name="hid_CallFile" value="impal-tran-dummy_ABCFMS.asp?">
...
<script>
    function dopart() {
        ...
        document.frmconsignment.action = "impal-tran-dummy_ABCFMS.asp"
        document.frmconsignment.method = "post"
        document.frmconsignment.submit("impal-tran-dummy_ABCFMS.asp")
    }
    ...
    function valacceptedqty(j) {
     ...
     function test1(j, k, m) {
        ...
        //	alert(acceptedqty);
       // reason = document.frmconsignment.item("text9", j).value;
        //	alert(reason);

        //	alert(m.value)
        //	alert(j)
        //	alert(k)
       // if ((Math.round(acceptedqty) > Math.round(toorderqty))) { // && (reason == '')) {
       //     alert("Since accepted quantity is greater than to order qty, Please Re-enter ")
       //     document.frmconsignment.item("text8", j).focus();
       // }
      /*   if (m.value == 'Others') {
            document.getElementById(k).innerHTML = '<INPUT type=text name=text9>'
            //document.frmconsignment.item("text9",j).focus
        }*/
     }
   ...
   function process() {
        ...
        //    if (parseInt(toorderqty) < parseInt(acceptedqty) )
        //    {
        //    //&& (document.frmconsignment.text9(i).value == '')) {

        //        //				alert("inside if")
        //        alert("Accepted qty greater than to order qty, Please select reason")
        //        //document.frmconsignment.text9(i).focus();
        //        document.frmconsignment.text8(i).focus();
        //        flag = false;
        //        break;
        ...
        //	alert(flag)
        if (flag) {
            document.frmconsignment.action = "impal-tran-POIndent_nbrBranches_new1_AbcFms_Nil.asp"
        ...
        /*if (flag)
        {	
        document.frmconsignment.action="impal-tran-POIndent_nbrBranches_new1.asp"
        document.frmconsignment.target="_self";
        document.frmconsignment.hidprocess.value = "P"	
        //		document.frmconsignment.submit();
        }*/
	...
	for (i=0;i<document.frmconsignment.text8.length;i++)
    {
    if((document.frmconsignment.text7(i).value < document.frmconsignment.text8(i).value) && (document.frmconsignment.text9(i).value == ''))
    {
    alert("Accepted qty greater than to order qty, Please select reason")
    document.frmconsignment.text9(i).focus()
    flag = false;
    ...
    if (flag)
    {	
 	document.frmconsignment.action= "SmartViewers-dummy.asp";
    document.frmconsignment.target="new";
    document.frmconsignment.submit();
    }
    }
    */
