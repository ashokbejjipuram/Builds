...
{
    public partial class ListingGST : System.Web.UI.Page
    {
        #region Declaration
        ...
        protected void LoadReportType()
        {
            ...
            try
            {
                ...
                ddlReportType.DataSource = oCommon.GetDropDownListValues("Finance-SalesTaxListing_GST");
        ...
        protected void btnReport_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                string str_head = "";
                btnReport.Text = "Back";
                string filename = "SalesTaxListing-GST_" + string.Format("{0:yyyyMMdd}", date1) + " to " + string.Format("{0:yyyyMMdd}", date2) + ".xls";

                SalesTransactions salesItem = new SalesTransactions();
                ds = salesItem.GetSalesTaxListingExcelDetails(Session["BranchCode"].ToString(), date1, date2);
                string strBranchName = (string)Session["BranchName"];
                str_head = "<center><b><font size='6'>Sales Tax Listing Report for the Period " + string.Format("{0:dd/MM/yyyy}", date1) + " to " + string.Format("{0:dd/MM/yyyy}", date2) + " of " + strBranchName + "</font></b><br><br></center>";
