...
{
    public partial class PurchaseReportSummary : System.Web.UI.Page
    {
        #region Declaration
        ...
        private void CallCrystalReport()
        {
            ...
            try
            {
                if (!string.IsNullOrEmpty(strBranchCode))
                {
                    ...
                    string strLineCodeQuery = "{purchase_report.supplier_name}";
                    string strBranchQuery = "{purchase_report.branch_code}";

                    if (ddlLineCode.SelectedIndex > 0)
                        strSelectionFormula = strLineCodeQuery + " = '" + ddlLineCode.SelectedItem + "'";
                    if (string.IsNullOrEmpty(strSelectionFormula))
                        strSelectionFormula = strBranchQuery + " = '" + strBranchCode + "'";
                    else if (!strBranchCode.Equals("CRP"))
