...
{
    public partial class PendingSTDNs : System.Web.UI.Page
    {
        #region Declaration
        ...
        protected void btnReportExcel_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                {
                    Server.ClearError();
                    Response.Redirect("PendingSTDNs.aspx", false);
                ...
                else
                {
                    ...
                    string str_head = "";
                    string filename = "PendingSTDNs_" + string.Format("{0:yyyyMMdd}", strToDate) + ".xls";

                    InwardTransactions inwardTransactions = new InwardTransactions();
                    ds = inwardTransactions.GetPendingSTDNsPlaced(ddlOrdBranchName.SelectedValue, strFromDate, strToDate);
                    str_head = "<center><b><font size='6'>Pending STDNs at Branches from " + string.Format("{0:dd/MM/yyyy}", strFromDate) + " to " + string.Format("{0:dd/MM/yyyy}", strToDate) + "</font></b><br><br></center>";
