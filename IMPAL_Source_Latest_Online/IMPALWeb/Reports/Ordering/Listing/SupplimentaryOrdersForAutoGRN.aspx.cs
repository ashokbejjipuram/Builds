...
{
    public partial class SupplimentaryOrdersForAutoGRN : System.Web.UI.Page
    {
        #region Declaration
        ...
        protected void btnReportExcel_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                {
                    Server.ClearError();
                    Response.Redirect("SupplimentaryOrdersForAutoGRN.aspx", false);
                ...
                else
                {
                    ...
                    string str_head = "";
                    string filename = "SuppliemtaryOrders_" + string.Format("{0:yyyyMMdd}", strToDate) + ".xls";

                    InwardTransactions inwardTransactions = new InwardTransactions();
                    ds = inwardTransactions.GetSuppliemrayOrderDetailsforAutoGRN(strBranchCode, strFromDate, strToDate);
                    str_head = "<center><b><font size='6'>Suppliemtary Orders for AUTO GRN from " + string.Format("{0:dd/MM/yyyy}", strFromDate) + " to " + string.Format("{0:dd/MM/yyyy}", strToDate) + "</font></b><br><br></center>";
